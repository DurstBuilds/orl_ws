...
- **Wheels move but knee/hip do not**: Hip uses **buttons 4/5** (not the knee stick). If `/knee_motor/motor_command` is non-zero but the joint stays still, the gateway may be dropping commands (internal `soft_mode=true` while teleop shows false). Temporarily launch with `gateway_debug_command_tx:=true` and check logs for `motor_command dropped` vs `TX path: MIT apply`. Compare `ros2 topic echo /knee_motor/motor_state` while moving the stick.
